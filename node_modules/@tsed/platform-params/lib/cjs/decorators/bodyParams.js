"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.RawBodyParams = exports.BodyParams = void 0;
const ParamTypes_1 = require("../domain/ParamTypes");
const useParam_1 = require("./useParam");
const mapParamsOptions_1 = require("../utils/mapParamsOptions");
function BodyParams(...args) {
    const { expression, useType, useConverter = true, useValidation = true } = (0, mapParamsOptions_1.mapParamsOptions)(args);
    return (0, useParam_1.UseParam)({
        paramType: ParamTypes_1.ParamTypes.BODY,
        expression,
        useType,
        useConverter,
        useValidation
    });
}
exports.BodyParams = BodyParams;
/**
 * RawBodyParams return the value from [request.body](http://expressjs.com/en/4x/api.html#req.body) as a Buffer.
 *
 * #### Example
 *
 * ```typescript
 * @Controller('/')
 * class MyCtrl {
 *    @Post('/')
 *    create(@RawBodyParams() body: Buffer) {
 *       console.log('Entire body', body.toString("utf8"));
 *    }
 * }
 * ```
 *
 * @decorator
 * @operation
 * @input
 * @alias BodyParams Example: @BodyParams() payload: Buffer
 */
function RawBodyParams() {
    return (0, useParam_1.UseParam)({
        paramType: ParamTypes_1.ParamTypes.RAW_BODY,
        useConverter: false,
        useValidation: false
    });
}
exports.RawBodyParams = RawBodyParams;
//# sourceMappingURL=bodyParams.js.map