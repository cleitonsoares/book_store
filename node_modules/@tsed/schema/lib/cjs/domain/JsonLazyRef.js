"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.JsonLazyRef = void 0;
const core_1 = require("@tsed/core");
const JsonSchemaMapperContainer_1 = require("../registries/JsonSchemaMapperContainer");
const getJsonEntityStore_1 = require("../utils/getJsonEntityStore");
class JsonLazyRef {
    constructor(getType) {
        this.getType = getType;
        this.isLazyRef = true;
    }
    get target() {
        return this.getType();
    }
    get schema() {
        return (0, getJsonEntityStore_1.getJsonEntityStore)(this.getType()).schema;
    }
    get name() {
        return (0, core_1.nameOf)(this.getType());
    }
    toJSON(options) {
        return this.getType() && (0, JsonSchemaMapperContainer_1.execMapper)("schema", this.schema, options);
    }
}
exports.JsonLazyRef = JsonLazyRef;
//# sourceMappingURL=JsonLazyRef.js.map